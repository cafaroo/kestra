...
public interface WorkerJobRunningStateStore {
     ...
     * <p>
     * Releasing a lease after having resubmitted the job races with the dispatch of that very job: the
     * queue can be Kafka, Redis or AMQP, whose messages are published immediately rather than with the
     * enclosing transaction, so the controller may already have dispatched the job and written a fresh
     * entry under the same key. Deleting that one would drop the lease of the worker now running it.
     *
     * @param key the key of the worker job to be deleted.
